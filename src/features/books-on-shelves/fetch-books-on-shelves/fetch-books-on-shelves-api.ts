...
import {
    FetchBooksOnUserShelvesResponseSchema,
    type FetchBooksOnUserShelvesErrorType,
} from './fetch-books-on-shelves-schema';

// Should I add the Zod Error Handler logic to the other api-features ?

export default async function fetchBooksOnUserShelvesAPI() {
    try {
        const result = await fetchWithAuth(
            `${import.meta.env.VITE_API_URL}/books-on-shelves/`,
            {
                method: 'GET',
            }
        );
        ...
        if (!result.ok) {
            let errorData: FetchBooksOnUserShelvesErrorType;
            try {
        ...
        const validatedData =
            FetchBooksOnUserShelvesResponseSchema.parse(jsonResponse);
        return validatedData;
    } catch (error) {
        if (error instanceof z.ZodError) {
            const zodError: FetchBooksOnUserShelvesErrorType = {
            ...
            throw error as FetchBooksOnUserShelvesErrorType;
        } else {
            const networkError: FetchBooksOnUserShelvesErrorType = {
                name: 'Network Error',
